...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      watch: { ignored: ['**/.figma/**'] },
      // Dev: proxy the API calls the live console makes to the FastAPI backend,
      // so the browser stays same-origin (no CORS). Prod serves both from FastAPI.
      proxy: {
        '/verify-resolution-rules': { target: 'http://127.0.0.1:8000', changeOrigin: true },
